...
class DataFetcher:
    ...
    def get_ohlc_data(self, pair, timeframe):
        ...
        try:
            ...
            if ohlc is None:
                raise ValueError("No se pudieron obtener datos OHLC.")
...
class IndicatorCalculator:
    @staticmethod
    def calculate_stochastic(ohlc_data, window=14):
        try:
            ...
            st.error(f"Error al calcular el estocástico: {e}")
            return ohlc_data

class ChartPlotter:
    @staticmethod
    def adjust_bar_width(selected_timeframe):
    ...
    @staticmethod
    def plot_candlestick_chart(ohlc_data, selected_pair, selected_timeframe):
        try:
            ohlc_data = IndicatorCalculator.calculate_stochastic(ohlc_data)
            ...
            width = ChartPlotter.adjust_bar_width(selected_timeframe)
            ...
            st.error(f"Error al trazar el gráfico: {e}")
            return None
